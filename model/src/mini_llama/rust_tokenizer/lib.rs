...
impl PyTokenizer {
    ...
    fn new(vocab_size: usize) -> Self {
        PyTokenizer {
            inner: BPETokenizer::new(vocab_size)
        }
    }

    fn train(&mut self, texts: Vec<String>, iterations: usize) -> PyResult<()> {
        self.inner.train(&texts, iterations)
...
impl BPETokenizer {
    ...
    pub fn train(&mut self, texts: &[String], iterations: usize) -> Result<()> {
        ...
        for i in 0..iterations {
            ...
            if self.vocab.len() >= self.vocab_size {
                println!("Reached maximum vocabulary size: {}", self.vocab_size);
                break;
            }

            // Getting pair frequencies in parallel
            let word_splits = Arc::new(word_splits.clone());
            let pairs = self.get_pair_frequencies(&word_splits);
            ...
            let merged = best_pair.0 + &best_pair.1;

            self.merges.insert(best_pair, merged.clone());
            self.vocab.insert(merged, self.vocab.len());

            if (i + 1) % 1000 == 0 {
                println!("Completed {} merges. Vocabulary size: {}", i + 1, self.vocab.len());
            }
        }
